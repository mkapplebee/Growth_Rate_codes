...
CssS-7 set, grown for 20hrs in LB adjusted to pH 5.0 with HCl
...
gcfile<-read.csv("C:/Users/Kenyon/Documents/Bacillus_subtilis/Growth Rates/Eon data/LB+HCL-PH5/LB_pH5-gcfit-repsavgd2016-02-10.csv")
...
epi_func(norm_dat,"CssS","McpA","CM",type="mult")
epi_func(norm_dat,"CssS","McpA","CM",type="add")

```

_Lag:_ All mutants have reduced lag (+fitness), but CM has less reduced lag than combined effects (negative epistasis).  
CM(-13) is between fittness of C(-18%) and M(-12), but is closer to M-phenotype - but not technically distinct from either.

_GR:_ No distinct phenotypes among strains

_maxA:_ negative synergistic epistasis - C & M have no distinct effect, while CM is distinctly reduced from C,M, and AM373

##CN:
```{r warning=FALSE, echo=FALSE,fig.height=8,fig.width=11,width=120}
options(width = 120)
errort.bars2(nrm3[-which(nrm3$strain=="McpA"|nrm3$strain=="CM"|nrm3$strain=="MN"),])
epi_func(norm_dat,"CssS","NhaK","CN",type="mult")
epi_func(norm_dat,"CssS","NhaK","CN",type="add")
```
